...
void collect_once()
{
	...
	while(DMA_GetFlagStatus(DMA1_FLAG_HT1)==0);//µÈ´ı²É¼¯Íê±Ï
...
void DMA_init()
{
    ...
    DMA_InitStructure.DMA_MemoryBaseAddr = (uint32_t)&adc_buff; //adcµÄ»º³åÇø
...
{ 	
	...
	ADC_InitStructure.ADC_ContinuousConvMode = ENABLE;	//Ä£Êı×ª»»¹¤×÷ÔÚµ¶à´Î×ª»»Ä£Ê½
